...
?>
    <div id="main">
        ...
        <form action="/user/sendMessageInDialog/<?php echo $id_interlocutor; ?>" method="post">
            <textarea name="text_message" class="textarea_text"></textarea><br />
            <input type="hidden" name="id_interlocutor" value="<?php echo $id_interlocutor; ?>" />
            <input type="submit" name="submit_message" value="Отправить сообщение" /><br /><br />
        </form>
        
        <?php  
            if (InterlocutorModel::countAllMessageInDialog($id_interlocutor) != 0) {
                foreach ($allMessageInDialogMas as $message) {
                ...
                echo '<p>Сообщений не существует</p>';
            }
       ?>
    </div>
